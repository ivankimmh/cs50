...
"""
- 이름 순서가 yearning 순서에 영향이 있음
- name 의 길이와 yearning 의 길이는 같음
- Photo 는 2 차원 배열이라 순서대로 result 에 담아서 결과 반환
...
def solution(name, yearning, photo):
    ...
    # name 배열과 yearning 배열을 합쳐서 dict 에 담기
    new_dict = {name[i]: yearning[i] for i in range(len(name))}

    # photo 를 돌면서 각각의 그리움 점수를 합산
    for pic in photo:
        score = sum(new_dict.get(friend, 0) for friend in pic)
        answer.append(score)
    # 그리움 점수 없는 경우 예외 처리
