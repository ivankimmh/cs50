...
넓은 벽 전체에 페인트를 새로 칠하는 대신, 구역을 나누어 일부만 페인트를 
새로 칠 함으로써 예산을 아끼려 합니다. 이를 위해 벽을 1미터 길이의 구역 n개로 나누고, 
...
페인트를 칠해도 되지만 다시 칠하기로 정한 구역은 적어도 한 번 페인트칠을 해야 합니다. 
...
1, 2, 3, 4번 구역에 페인트칠을 하면 2번 만에 다시 칠해야 할 곳에 
모두 페인트칠을 할 수 있습니다.
...
def solution(n, m, section):
    # n = 자연수인 리스트 len(n) = n - 1
    # m 이 커버해야하는 길이를 구해야한다 ?
    # section 리스트를 돌면서 첫번째 숫자부터 커버되는 구간을 찾아야 한다.
    # section[0] 에서 부터 m - 1 숫자까지 커버가 되어야 한다.
    # section[i] 기준이 되겠지
    ...
    while i < len(section):
        # 첫번째 부터 한번 칠하면 커버되는 구간
        covered_num = section[i] + m - 1

        # 한번 칠했을 때 커버되는 부분이 있다면 넘어가고
        # section 리스트의 길이를 i 가 초과하지 못하게
        while i < len(section) and section[i] <= covered_num:
            i += 1
